...
支持文生图的大模型： `OpenAI`、`QianFan`、`智谱 AI` 等等。

![文生图大模型](materials/text-to-image-model.png)

PS: 本次文生图的前置知识点是 “Spring AI 整合智谱AI”，若不清楚的话可以参考 [12-multi-model](../12-MultiModel/README.MD)。
...
`Spring AI` 官网中明确提到已经为 `智谱 AI` 提供了接口实现类 `ZhiPuAiImageModel`。
...
![img.png](materials/first-result.png)

生成的图片如下所示：

![img.png](materials/gen-image.png)
