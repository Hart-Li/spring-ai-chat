...
![SpringAI支持TTS的大模型](materials/spring-ai-tts-supported-models.png)
...
1. 注册（略）
2. 开通模型服务
![免费体验](materials/free.png)
![领取免费体验额度](materials/get-free-quota.png)
![领取免费体验额度第二步](materials/get-free-quota-2.png)
3. 创建 API-Key
![api-key manage](materials/api-key-manage-page.png)
![create-api-key](materials/create-api-key.png)
...
测试结果：
![测试结果](materials/first-result.png)

访问该地址会让你下载一个音频文件，下载到本地后访问即可。

PS: 这里遇到一个坑， springboot 与 spring-ai 产生了冲突，因此做了如下修改：

1. 本 Module 开始 pom.xml 中不再指定 springboot-starter-web 的版本
...
2. 在 父(根目录下) 的 pom.xml 中指定 springboot 的统一版本
