...
注意：并非所有模型都能支持所有类别的工具调用，详细情况参考 [模型对比](https://docs.spring.io/spring-ai/reference/api/chat/comparison.html)，也可以根据[第 2 章](../02-ChatModel/README.MD)的不同供应商的支持能力来做对比。

实测：DeepSeek（OpenAI-proxy）支持信息检索工具，采取行动工具受限。信息检索工具调用需要依赖用户侧集成（用户提供自定义工具）。

![DeepSeek 支持的工具](materials/ds-support-tool-classify.png)
...
测试路由：<br/>
对比两个路由，看看有无工具的效果<br/>
1. http://localhost:8080/ai/chat/deepseek?question=现在的时间是？

测试结果：
![无tools的结果](materials/ds-without-tool-result.png)

2. http://localhost:8080/ai/chat/tools?question=现在的时间是？

测试结果：
![有tools的结果](materials/ds-with-tool-result.png)
...
再来测试一下，这次我们使用之前无指定 tool 的接口：http://localhost:8080/ai/chat/deepseek?question=现在的时间是？

测试结果：
![使用全局的tool的结果](materials/ds-with-global-tool-result.png)
...
测试路由：<br/>http://localhost:8080/ai/chat/deepseek?question=设定一个今晚8点的闹钟

测试结果：

![采取行动工具结果](materials/action-tools-result.png)
