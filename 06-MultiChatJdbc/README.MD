...
![预制的表结构文件](materials/pre_prepared_db_schema.png)
...
![MySQL的测试结果](materials/mysql-test-result.png)
...
```
测试路由：
1. http://localhost:8080/ai/chat/deepseek/param?question=我是HartLi，是一名Java架构师，知识分享者，请记住我，以便我们后面的对话&conversationId=1234567890
2. http://localhost:8080/ai/chat/deepseek/param?question=我是谁&conversationId=1234567890

DB 数据如下：
![MySQL测试数据](materials/mysql-test2-1-result.png)

重启项目后继续测试：
1. http://localhost:8080/ai/chat/deepseek/param?question=我是谁&conversationId=1234567890

此时仍然可以知道我的身份。

![重启服务后MySQL测试数据](materials/mysql-test2-2-result.png)